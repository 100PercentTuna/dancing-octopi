 ...
 * - Data Reference: Statistics with source citation
 */
...
(function(domReady) {
    ...
    function initInlineFormats() {
        if (typeof wp === 'undefined' || 
            ...
            return; // Silently fail if dependencies not available
        }
    ...
    } // End initInlineFormats
    
    // Try to initialize - WordPress should be available when this script loads
    if (typeof domReady === 'function') {
        domReady(initInlineFormats);
    } else if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initInlineFormats);
    } else {
        // DOM already ready, try immediately
        initInlineFormats();
    }
    
})(window.wp && window.wp.domReady ? window.wp.domReady : null);
